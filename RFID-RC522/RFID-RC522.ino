/*
 * File nameï¼šRFID.pde
 * Creatorï¼šDr.Leong   ( WWW.B2CQSHOP.COM )
 * Creation dateï¼š2011.09.19
 * Modified byï¼š Eng. Robson (robson.eletronico@gmail.com)
 * Modified dateï¼š 2013.09.10
 * Modifiedï¼š Translation from Chinese to English (by google)
 * Functional Descriptionï¼šMifare1 Anti-collision find cards â†’ â†’ â†’ Select card reader interface
 */
...
int redLED = 5;           // the pin that the LED is attached to
int greenLED = 4;           // the pin that the LED is attached to
int blueLED = 3;           // the pin that the LED is attached to
...
void loop()
{
  ...
  if (myservo.read() != 93) {
    ...
    myservo.write(93);                  // sets the servo position according to the scaled value
  }
  ...
  if (status == MI_OK)
  {
    ...
    Serial.println("The card's number array is: ");
    //Serial.print(2);
    ...
    } else if (serNum[0] == 131) {
      analogWrite(greenLED, 255);
      analogWrite(redLED, 0);
      analogWrite(blueLED, 0);
      Serial.println("Hello Keychain");
      //Serial.print(myservo.read());
      myservo.write(180);                  // sets the servo position according to the scaled value
      delay(3000);                           // waits for the servo to get there (user to enter)
    }
    analogWrite(blueLED, 255);
    analogWrite(greenLED, 0);
    analogWrite(redLED, 0);
